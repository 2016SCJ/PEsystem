...
//	private HibernateTemplate hibernateTemplate;
	...
	public List<Object[]> queryAllStartClass() {
		String queryString="select sc.id, c.couName, t.teaName, sc.claLocation, sc.claTime, sc.claCapacity, sc.claRemain from StartClass sc left join sc.course c left join sc.teacher t";
...
//        return entitylist;
	...
	public boolean chooseClass(String stuNo, Integer startClassId) {
		...
		if(sessionFactory.getCurrentSession().createQuery("from Score s inner join fetch s.student inner join fetch s.startClass where s.student.stuNo=:stuNo and s.startClass.id=:startClassId").setParameter("stuNo", stuNo).setParameter("startClassId", startClassId).list().size()==1)
			return false;
		
		startClass.setClaRemain(startClass.getClaRemain()-1);
		sessionFactory.getCurrentSession().update(startClass);
